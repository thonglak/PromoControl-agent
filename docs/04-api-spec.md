...
GET    /api/monitor-links                          (list ทุกลิงค์ + project list ในแต่ละลิงค์)
POST   /api/monitor-links                          (สร้าง — body: { name, project_ids:[] })
PUT    /api/monitor-links/{id}                     (แก้ name + project_ids)
DELETE /api/monitor-links/{id}                     (ลบ)

# Public — ไม่ต้อง auth (ผ่าน api/public/* exempt ใน JwtAuthFilter)
GET    /api/public/monitor/{token}                 (ดู KPI หลายโครงการผ่าน token link)
...
## Units
GET /api/units?project_id=&search=&house_model_id=&status=
                                #   search รับได้ทั้งค่าเดียว (LIKE รหัส/เลขที่ยูนิต)
                                #   และหลายค่า: วางรายการคั่นด้วย , ; เว้นวรรค หรือขึ้นบรรทัดใหม่
                                #   (วงเล็บถูกตัดทิ้ง) → ค้น unit_number แบบตรงตัว (IN)
                                #   เรียงผลตามลำดับที่กรอก (ORDER BY FIELD)
GET /api/units/export          ← ส่งออก Excel (.xlsx) รองรับ filter เดียวกับ list (รวม multi-search)
GET /api/units/{id}
POST /api/units
PUT /api/units/{id}
DELETE /api/units/{id}
POST /api/units/bulk-house-model        # อัปเดต house_model_id ของหลายยูนิตพร้อมกัน — role: admin, manager
                                #   body: { unit_ids: number[], house_model_id: number|null }
                                #   house_model_id = null → ล้างแบบบ้าน
                                #   ยูนิต sold/transferred แก้ได้เฉพาะ admin (non-admin → ข้าม)
                                #   ยูนิตที่เป็นแบบบ้านนั้นอยู่แล้ว → ข้าม
                                #   response data: { updated, skipped:[{unit_code,reason}] }
...
POST   /api/promotion-items/import-json           (นำเข้าจากไฟล์ JSON ที่ export มา — project_id, items[]; resolve eligible_house_model_names/eligible_unit_codes ตามโครงการปลายทาง; ตรวจรายการซ้ำด้วย "ชื่อ" — ชื่อซ้ำในโครงการจะถูกข้าม; รหัส code สร้างใหม่อัตโนมัติเสมอ (ไม่ใช้รหัสจากไฟล์); สูงสุด 500 รายการ/ครั้ง)
...
### GET /api/sales-transactions — computed per-row fields

แต่ละ row ใน list response มี field ที่ API คำนวณเติม (derive จาก budget_movements ledger — single source of truth):
- `total_budget_remaining` (X) — งบคงเหลือรวมของยูนิต ณ รายการนี้ (รวมทุก source); `null` สำหรับ row `status=legacy`
- `net_extra_budget_used` (Y) — งบนอกสุทธิที่ใช้ = งบอื่นที่ใช้ − งบยูนิตคงเหลือ (รวมค่าธรรมเนียมโอน mode `as_premium`); `0` ถ้าไม่ติดลบ, `null` สำหรับ legacy
- `management_budget_allocated` (Quota) — งบผู้บริหารที่จัดสรร (`MANAGEMENT_SPECIAL` allocated) ของยูนิต; `null` สำหรับ legacy
...
> หมายเหตุ: ระบบไม่มีขั้นตอนรออนุมัติงบแล้ว — budget_movements ทุกรายการถูกบันทึกเป็น
> `status = approved` ทันที (endpoint `:id/approve` และ `:id/reject` ถูกถอดออก)
