...
export class SalesEntryService {
  ...
  getTransactions(params: Record<string, unknown> = {}): Observable<{ data: SalesTransaction[]; total: number; page: number; per_page: number; summary?: { unit_budget_used: number; unit_budget_remaining: number; pool_budget_used: number; pool_budget_remaining: number; management_budget_used: number; management_budget_remaining: number; management_budget_returned: number; total_budget_remaining_all_units: number; total_profit: number; legacy: { total_budget_remaining: number; total_profit: number; as_of_date: string; note: string | null } | null } }> {
